Data_drinking_water <- read.csv("20230801_drinking_safety_basic_separted.csv")
...
#GDP per capital
Data_GDP <- select(Data_drinking_water, Country, SC_2000, SC_2020, GDPcapital2000, GDPcapital2020, Dummy0, Dummy1)
Data_GDP <- Data_GDP %>% drop_na()


Data_GDP_2000 <- select(Data_GDP, Country, SC_2000, GDPcapital2000, Dummy0)
colnames(Data_GDP_2000) <- c("country", "coverage", "GDPcapital", "Dummy") 

Data_GDP_2020 <- select(Data_GDP, Country, SC_2020, GDPcapital2020, Dummy1)
...
ggsave(path = NULL, file = "GDP_drinking_safe_20230801.svg", plot = plot, dpi = 600, width = 4, height = 3.2)
...
#Net ODA
Data_ODA <- select(Data_drinking_water, Country, Coverage2000, Coverage2020, NetODA2020, NetODA2000, Dummy_2000, Dummy_2020)
...
#urbanization rate
Data_UP <- select(Data_drinking_water, Country, SC_2000, SC_2020, UP2000, UP2020, Dummy0, Dummy1)
Data_UP <- Data_UP %>% drop_na()

Data_UP_2000 <- select(Data_UP, Country, SC_2000, UP2000, Dummy0)
colnames(Data_UP_2000) <- c("country", "coverage", "UP", "Dummy") 

Data_UP_2020 <- select(Data_UP, Country, SC_2020, UP2020, Dummy1)
...
ggsave(path = NULL, file = "UP_drinking_safe_20230801.svg", plot = plot_UP, dpi = 600, width = 4, height = 3.2)
...
#demogracy index
Data_di <- select(Data_drinking_water, Country, SC_2000, SC_2020, DI2006, DI2020, Dummy0, Dummy1)
Data_di <- Data_di %>% drop_na()

Data_di_2000 <- select(Data_di, Country, SC_2000, DI2006, Dummy0)
colnames(Data_di_2000) <- c("country", "coverage", "di", "Dummy") 

Data_di_2020 <- select(Data_di, Country, SC_2020, DI2020, Dummy1)
...
ggsave(path = NULL, file = "di_drinking_safe_20230801.svg", plot = plot_di, dpi = 600, width = 4, height = 3.2)
...
Data_RW <- select(Data_drinking_water, Country, SC_2000, SC_2020, RW2000, RW2020, Dummy0, Dummy1)
Data_RW <- Data_RW %>% drop_na()

Data_RW_2000 <- select(Data_RW, Country, SC_2000, RW2000, Dummy0)
colnames(Data_RW_2000) <- c("country", "coverage", "RW", "Dummy") 

Data_RW_2020 <- select(Data_RW, Country, SC_2020, RW2020, Dummy1)
...
ggsave(path = NULL, file = "RW_drinking_safe_20230801.svg", plot = plot_RW, dpi = 600, width = 4, height = 3.2)
